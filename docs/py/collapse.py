...
energy boundaries and any number of uniform or custom angle bins.

Uses uniform (width-weighted) overlap-fraction averaging for both input and
output energy groups, and width-proportional fractional overlap for angle bins.
Fully vectorized with numpy einsum — no Python loops over groups.
"""
...
def _collapse_to_array(
    ...
    *,
    ...
    Uses uniform overlap-fraction weighting for both input and output energy
    groups. No sub-group reconstruction or kinematic assignment — just
    width-proportional averaging.
    ...
    w_in = f_e * fine_widths[:, None]
    coarse_widths = w_in.sum(axis=0)

    result = np.einsum("gI,gpA,pJ->IJA", w_in, sigma_a, f_e)

    safe_widths = np.where(coarse_widths > 0, coarse_widths, 1.0)
    result /= safe_widths[:, None, None]
...
def collapse(
    ...
    *,
    ...
    n_angle_bins : int, optional
        ...
        Mutually exclusive with ``angle_boundaries``.
    angle_boundaries : list[float], optional
